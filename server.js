...
<head>
  ...
  <style>
    ...
    .tab-btn {
      ...
      padding: 12px 24px; border-radius: 30px; cursor: pointer; font-weight: 600;
    }
    ...
    .content-card { background: var(--card-bg); border: 1px solid var(--border-color); border-radius: 16px; padding: 30px; }
    ...
    .result-box { margin-top: 30px; padding: 20px; background: #fff; border: 1px solid var(--border-color); border-radius: 12px; display: none; text-align: center; }
...
<body>
  <div class="container">
    ...
    <div class="tabs">
      <button class="tab-btn active" onclick="switchTab('template', event)">🖼️ 템플릿 기반</button>
      <button class="tab-btn" onclick="switchTab('longform', event)">🔗 AI 롱폼 추출</button>
      <button class="tab-btn" onclick="switchTab('script', event)">✍️ 대본 기반 TTS</button>
    </div>
  ...
  <script>
    ...
    function updateLivePreview(progress) {
      ...
      if(theme === 'purple') {
        ...
        grad.addColorStop(0, '#111827'); grad.addColorStop(1, '#374151');
      }
      
      ctx.fillStyle = grad;
      ctx.fillRect(0, 0, 720, 1280);

      var circleY = 640 + Math.sin(progress * Math.PI * 4) * 50;
